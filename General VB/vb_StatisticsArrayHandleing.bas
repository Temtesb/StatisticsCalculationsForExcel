Attribute VB_Name = "vb_StatisticsArrayHandleing"
Option Explicit
...
'jeremy.gerdes@navy.mil
    ...
    Public Function IsExcelRangeValidArray(ByRef obj As Variant)
        Dim oExcel As Object: Set oExcel = CreateObject("Excel.Application")
        Dim ary() As Variant
        ary = obj.CurrentArray
        If IsArray(ary) And ArrayRank > 0 Then
            IsExcelRangeArray = True
        End If
    End Function
    ...
    Public Function ModeSingleFromArray(ByRef ary As Variant) As Double 'single dimention array
    'If multiple modes exists the first one found will be returned,
    'this appears to be the behaviour of Excels 'Mode.SNGL' function
        'TODO...
    End Function
    
    Public Function ModeMultiFromArray(ByRef ary As Variant) As Variant 'single dimention array
    'If multiple modes exists returns an array as a list of all existing modes
    'this appears to be the behaviour of Excels 'Mode.SNGL' function
        'TODO...
    End Function
